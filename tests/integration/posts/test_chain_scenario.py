...
class TestSocialChain:
    ...
    def test_like_post_synchronization(self, user_factory, setup_teardown_user, api_client):
        # ==================== 1. 角色 A 准备与发帖 ====================
        with allure.step("1. 角色 A 注册并登录"):
            user_a = user_factory()
            api_client.post("/api/v2/user/register", json=user_a)
            setup_teardown_user.append(user_a["username"])
            
            resp_a = api_client.post("/api/v2/user/login", json=user_a)
        ...
        with allure.step("6. 角色 A 查询列表，验证赞数一致性"):
            resp_list = api_client.get("/api/v2/posts/list", headers=header_a)
